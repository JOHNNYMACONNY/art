...
func _run() -> void:
	...
	# Keep the 01O candidate-selection contract inside this exact-head runtime gate before generic output probes.
	var yardline_interstitial_candidate_error: String = YardlineRadioInterstitialCandidateSelectionContract.verify()
	if not yardline_interstitial_candidate_error.is_empty():
		await _fail("Audio Production 01O candidate: %s" % yardline_interstitial_candidate_error)
		return
	await process_frame
