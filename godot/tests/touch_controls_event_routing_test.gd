...
func _run() -> void:
	...
	if packed == null:
		_fail("Could not load scrap_test_block.tscn")
		return
	...
	var player := scene.get_node_or_null("Runner")
	if touch_ui == null or player == null:
		_fail("Main scene is missing TouchControlsUI or Runner")
		return
	...
	if _last_joystick_vector.length() <= 0.05:
		_fail("A left-half touch drag did not reach the floating joystick through the real Viewport GUI route")
		return
	if player.joystick_vector.length() <= 0.05:
		_fail("Joystick signal did not reach PlayerRunner")
		return
	...
	if player.joystick_vector.length() > 0.001:
		_fail("Joystick release did not clear PlayerRunner input")
		return
