...
func _run() -> void:
	...
	for method_name in ["_process_open_world_wanted", "_has_open_world_direct_observation", "_begin_disturbance_sequence"]:
		if not _scene_under_test.has_method(method_name):
			await _fail("Main scene missing open-world Wanted seam: %s" % method_name)
			return
	...
	var alarm := _scene_under_test.get_node_or_null("GearsDistrictSlice01B/IndustrialFrontage/CivicServiceAlarm")
	var wanted_label := _scene_under_test.get_node_or_null("CanvasLayer/WantedStatusLabel") as Label
	var authority = _scene_under_test.get("wanted_authority")
	if player == null or pursuer == null or alarm == null or wanted_label == null or authority == null:
		await _fail("Real production scene is missing Runner, Pursuer, CivicServiceAlarm, WantedStatusLabel, or WantedAuthority")
		return
	...
	if not bool(_scene_under_test.call("_has_open_world_direct_observation", pursuer, player)):
		await _fail("Expected clear Gears primary-road observation fixture is occluded")
		return
	_scene_under_test.call("_process_open_world_wanted", 0.1)
	...
	_scene_under_test.call("_process_open_world_wanted", 1.0)
	_scene_under_test.call("_process_open_world_wanted", 1.0)
	...
	if bool(_scene_under_test.call("_has_open_world_direct_observation", pursuer, player)):
		await _fail("Commercial frontage did not block the production observation ray")
		return
	for _i in range(3):
		_scene_under_test.call("_process_open_world_wanted", 0.4)
	...
	if not wanted_label.visible or not wanted_label.text.contains("SEARCH"):
		...
		return
	var search_anchor = _scene_under_test.get("_wanted_search_anchor")
	if search_anchor == null or pursuer.get("target_node") != search_anchor:
	...
	_scene_under_test.call("_process_open_world_wanted", 0.2)
	...
	if _state(authority) != "SEARCH":
		...
		return
	_scene_under_test.call("_process_open_world_wanted", 7.0)
	if _heat(authority) != 0 or _state(authority) != "CLEAR" or wanted_label.visible:
