...
func _run_v8_m25_echo_radio_interference_assertions() -> void:
	...
	var procedural_fallback = audio_mgr._create_fractured_carrier_wav(1.0, 0.3)
	assert(procedural_fallback != null, "FAIL 9: Procedural fallback remains independently synthesizable")
	assert(procedural_fallback.format == AudioStreamWAV.FORMAT_8_BITS, "FAIL 9: Procedural fallback remains 8-bit")
	assert(procedural_fallback.mix_rate == 22050, "FAIL 9: Procedural fallback remains 22050 Hz")
	assert(procedural_fallback.loop_mode == AudioStreamWAV.LOOP_FORWARD, "FAIL 9: Procedural fallback remains loopable")
	assert(is_equal_approx(procedural_fallback.get_length(), 1.0), "FAIL 9: Procedural fallback remains exactly 1.0s")
	print("  -> Assertion 9 PASS: Production interference metadata/playback and independent procedural fallback verified!")
