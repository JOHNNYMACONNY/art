class_name PropSecurityCheckpoint
extends StaticBody3D

@export var material_main: Material
@export var material_sensor: Material
...
func _setup_materials() -> void:
	...
	for m in meshes:
		...
		for s in range(surf_count):
			if s == 1 and material_sensor:
				mi.set_surface_override_material(s, material_sensor.duplicate())
			elif material_main:
