...
const PRODUCTION_PATH := "user://burnside_mayor_burn_contact.json"
...
func _resolve_default_storage_path() -> String:
	for arg in OS.get_cmdline_args():
		...
		if text.contains("res://tests/") and text.ends_with(".gd"):
			return "%s/p10_%s.json" % [TEST_DIRECTORY, text.get_file().get_basename()]
...
func _load_from_disk() -> void:
	...
	if not document.has("version") or not document.has("mayor_burn_state"):
		_write_blocked = true
		_load_status = "MALFORMED"
		return
	...
	if typeof(document["mayor_burn_state"]) != TYPE_STRING:
		_write_blocked = true
		_load_status = "MALFORMED"
		return
	var state_name := String(document["mayor_burn_state"])
	match state_name:
		"UNESTABLISHED":
			_state = ContactState.UNESTABLISHED
		"KNOWN":
...
func _persist() -> bool:
	...
	var payload := {
		...
		"mayor_burn_state": get_state_name(),
	}
